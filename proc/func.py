...
def get_color_patchwise(impath:str,w_patch:int,h_patch:int, area:Rect):
    res_arr = np.zeros(8)
    m_img_bgr = cv2.imread(impath)
    m_img_hsv = cv2.Mat(m_img_bgr.shape)
    m_img_lab = cv2.Mat(m_img_bgr.shape)
    cv2.cvtColor(m_img_bgr,m_img_hsv,cv2.COLOR_BGR2HSV)
    cv2.cvtColor(m_img_bgr,m_img_hsv,cv2.COLOR_BGR2Lab)
    ...
    for x in range(num_w):
        for y in range(num_h):
            ...
            np.append(res_arr,c_arr,axis=0)
